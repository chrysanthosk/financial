...
class ImportController extends Controller
{
    ...
    public function preview(Request $request, string $type)
    {
        ...
        if ($type === 'income') {
            ...
            if (!$dateCol) return back()->with('error', 'Please select the Date column.');
            ...
            if (count($rows) === 0) return back()->with('error', 'No rows to preview.');
    ...
    public function commit(Request $request, string $type)
    {
        ...
        if ($failed > 0) $msg .= " Failed {$failed} row(s) (unexpected).";
    ...
    private function normalizeType(string $type): string
    {
        ...
        if (in_array($type, ['income', 'incomes'], true)) return 'income';
        if (in_array($type, ['expense', 'expenses'], true)) return 'expenses';
        abort(404);
    }
    ...
    private function validateMapping(string $type, array $mapping): void
    {
        foreach (array_keys($this->requiredFields($type)) as $f) {
            if (empty($mapping[$f])) abort(422, "Missing mapping for required field: {$f}");
        }
    }
    ...
    private function readHeadersAndSample(string $storedPath, bool $hasHeader): array
    {
        ...
        if ($hasHeader) {
            ...
            foreach ($first as $col => $_val) $headers[$col] = $col;
    ...
    private function readAllRows(string $storedPath, bool $hasHeader): array
    {
        ...
        if ($hasHeader && !empty($rows)) array_shift($rows);

        return array_values(array_filter($rows, function ($r) {
            foreach ($r as $v) {
                if (trim((string)$this->stringifyCell($v)) !== '') return true;
            }
            return false;
        }));
    }
    ...
    {
        if ($value instanceof RichText) return $value->getPlainText();
        return (string)$value;
    }
    ...
    private function normalizeDateValue($value, ?int $fallbackYear = null): ?string
    {
        ...
        foreach ($formatsWithYear as $f) {
            try { return Carbon::createFromFormat($f, $s)->toDateString(); } catch (\Throwable $e) {}
        }
    ...
    private function validateExpenseRows(array $rows, array $mapping): array
    {
        ...
        foreach ($rows as $idx => $row) {
            ...
            if ($amount === null || $amount <= 0) $errors[] = "Invalid amount";

            $payee = trim($this->stringifyCell($row[$mapping['payee_name']] ?? ''));
            if ($payee === '') $errors[] = "Missing payee/company";
            ...
            if (!$methodId) $methodId = $defaultMethodId;
            ...
            if (!empty($mapping['cheque_no'])) {
                ...
                if ($cheque !== '') $data['cheque_no'] = $cheque;
            }
